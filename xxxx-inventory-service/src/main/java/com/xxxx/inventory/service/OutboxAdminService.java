...
/**
 * Service admin để xem, replay hoặc ignore DLQ nội bộ của inventory-service.
 *
 * <p>Inventory outbox phát event sau khi giữ/hoàn tồn kho. Khi Kafka lỗi kéo dài, record chuyển sang {@code FAILED} để
 * không retry vô hạn. Admin có thể replay sau khi sửa lỗi hạ tầng hoặc ignore nếu event không còn cần gửi.</p>
 */
@Service
...
public class OutboxAdminService {
    ...
    public OutboxRecordResponse replay(Long id) {
        InventoryEventOutboxEntity event = getEvent(id);
    ...
    public OutboxMetricsResponse metrics() {
        ...
        LocalDateTime oldestFailed = outboxRepository.findFirstByStatusOrderByCreatedAtAsc(STATUS_FAILED)
                .map(InventoryEventOutboxEntity::getCreatedAt)
                .orElse(null);
        Integer maxAttemptCount = outboxRepository.findTopByOrderByAttemptCountDesc()
                .map(InventoryEventOutboxEntity::getAttemptCount)
                .orElse(0);
    ...
    private InventoryEventOutboxEntity getEvent(Long id) {
        return outboxRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Không tìm thấy outbox record id=" + id));
    }

    private OutboxRecordResponse toRecordResponse(InventoryEventOutboxEntity event) {
