...
import com.xxxx.payment.controller.dto.request.OutboxResolveRequest;
...
/** API admin để vận hành outbox/DLQ nội bộ của payment-service. */
@RestController
@RequestMapping("/api/payment/admin/outbox")
@RequiredArgsConstructor
@Tag(name = "Payment Outbox Admin", description = "API admin xem, replay và resolve outbox DLQ của payment-service")
public class OutboxAdminController {

    private final OutboxAdminService outboxAdminService;

    @Operation(summary = "Liệt kê outbox FAILED", description = "Trả danh sách record payment outbox đã rơi vào DLQ")
    @GetMapping("/failed")
