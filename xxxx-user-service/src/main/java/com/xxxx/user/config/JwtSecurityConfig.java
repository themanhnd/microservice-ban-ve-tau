...
public class JwtSecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(
        ...
        JwtAuthenticationFilter jwtFilter = new JwtAuthenticationFilter(
                jwtSecret,
                jwtIssuer,
                List.of("/api/users/login", "/api/users/register", "/api/users/refresh", "/api/users/logout", "/actuator", "/swagger-ui", "/v3/api-docs")
        );
        ...
        return http
                ...
                .authorizeHttpRequests(auth -> auth.requestMatchers("/api/users/login", "/api/users/register", "/api/users/refresh", "/api/users/logout", "/actuator", "/swagger-ui", "/v3/api-docs").permitAll().anyRequest().authenticated())
